...
namespace UnitTest1
{
	...
	public:
		...
		{
			std::array<std::size_t, 4> sol = {1,2,3,4};
		...
		TEST_METHOD(TestEvaluate)
		{
			...
			ColorCode first(std::array<std::size_t, 4>{2, 5, 2, 1});
		...
		TEST_METHOD(TestEvaluate2)
		{
			...
			ColorCode second(std::array<std::size_t, 4>{1, 1, 6, 2});
		...
		TEST_METHOD(TestGetSolution)
		{
			...
			ColorCode cc(std::array<std::size_t, 4>{2, 5, 2, 1});

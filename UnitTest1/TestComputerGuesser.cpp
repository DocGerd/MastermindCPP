...
namespace Microsoft {
	namespace VisualStudio {
		namespace CppUnitTestFramework {
			template<> static std::wstring ToString<ColorCode>(const ColorCode& q) {
				return q.toString();
			}
		}
	}
...
namespace UnitTest1
{
	...
	public:
		TEST_METHOD(TestRunCheat)
		{
			...
			ColorCode* expected = new ColorCode(std::array<int, 4> { 0, 0, 0, 0 });
		...
		TEST_METHOD(TestRunNormal)
		{
			...
			delete expected;
			delete guess;
			expected = new ColorCode(std::array<int, 4> { 1, 1, 1, 1 });
			guess = cg.nextGuess();
			Assert::AreEqual(*expected, *guess, L"", LINE_INFO());
			...
			delete expected;
			delete guess;
			expected = new ColorCode(std::array<int, 4> { 1, 2, 3, 4 });
			guess = cg.nextGuess();
			Assert::AreEqual(*expected, *guess, L"", LINE_INFO());
			...
			delete expected;
			delete guess;
			expected = new ColorCode(std::array<int, 4> { 2, 1, 3, 5 });
			guess = cg.nextGuess();
			Assert::AreEqual(*expected, *guess, L"", LINE_INFO());
			...
			delete expected;
			delete guess;
			expected = new ColorCode(std::array<int, 4> { 4, 3, 1, 2 });
			guess = cg.nextGuess();
			Assert::AreEqual(*expected, *guess, L"", LINE_INFO());
			...
			delete expected;
			delete guess;
			expected = new ColorCode(std::array<int, 4> { 4, 3, 1, 4 });
			guess = cg.nextGuess();
			Assert::AreEqual(*expected, *guess, L"", LINE_INFO());
			...
			delete expected;
			delete guess;
			expected = new ColorCode(std::array<int, 4> { 4, 3, 1, 0 });
			guess = cg.nextGuess();
			Assert::AreEqual(*expected, *guess, L"", LINE_INFO());
